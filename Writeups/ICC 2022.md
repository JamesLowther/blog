...
In June of 2022 I was invited to play for CyberSCI Team Canada at the first International CyberSecurity Challenge, hosted by ENISA in Athens, Greece. Here are a few of the challenges I solved, some of the retroactively, at the event.
...
Initial attempts failed until we realized that the binaries installed on the remote are busybox. This means they don't include all flags on a fully-functional binary. We used the following docs to help build the payloads: https://boxmatrix.info/wiki/Property:tar.
...
A similar process was used to determine that `flag.txt` exists in the `/flag` directory. It involved pipeing the output of `ls` to a file and downloading it.
